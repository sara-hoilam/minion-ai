...
# Composer / Auto token rates (USD per million tokens) — Cursor list pricing.
_INPUT_USD_PER_M = 1.25
_OUTPUT_USD_PER_M = 6.0
_CACHE_READ_USD_PER_M = 0.25
_CACHE_WRITE_USD_PER_M = 1.25
...
def monthly_usage_usd(user_id: int, *, period_start=None, period_end=None) -> float:
    q = db.session.query(db.func.coalesce(db.func.sum(LlmUsageEvent.cost_usd), 0)).filter(
...
def record_cursor_run(
    ...
    *,
    ...
    if cost > 0:
        ...
        if sub:
            sub.token_used_usd = float(sub.token_used_usd or 0) + cost
            sub.updated_at = _now()

    db.session.commit()
    logger.info(
        "Logged LLM usage user=%s run=%s tokens=%s cost_usd=%s",
        user_id,
        run_id,
        total,
        cost,
...
def sync_subscription_usage_from_events(user: User) -> UserSubscription | None:
    """Reconcile subscription token_used_usd from logged events in the billing period."""
    sub = UserSubscription.query.filter_by(user_id=user.id).first()
    if not sub:
        return None
    total = monthly_usage_usd(
        user.id,
