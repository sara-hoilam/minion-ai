...
def _run_job(job_id: str, build_fn: Callable[[FrameworkBuildReporter], dict]) -> None:
...
def start_job(user_id: int, build_fn: Callable[[FrameworkBuildReporter], dict]) -> str:
    ...
    thread = threading.Thread(target=_run_job, args=(job_id, build_fn), daemon=True)
    thread.start()
    return job_id
